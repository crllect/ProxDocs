...
export const isInternal = (url: string) =>
	typeof url === "string" &&
	url.slice(0, "astropreact:".length).toLowerCase() ===
		"astropreact:";
...
export const listPages = () =>
	[...pages.entries()].map(([name, def]) => ({
		name,
		title: def.title,
		url: `astropreact://${name}`
	}));

export const scheme = "astropreact";
export const homeUrl = `astropreact://home`;
...
const errorDocument = (name: string) =>
	wrap(
		...
		`<main class="internal">
       <h1>No such page</h1>
       <p><code>astropreact://${escapeHtml(name)}</code> does not exist.</p>
       <ul>${listPages()
			.map(
				page =>
