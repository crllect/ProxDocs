//#if vite
import cssText from "./styles.{{STYLE_EXT}}?inline";
//#endif
...
export const isInternal = (url: string) =>
	typeof url === "string" &&
	url.slice(0, "{{INTERNAL_SCHEME}}:".length).toLowerCase() ===
		"{{INTERNAL_SCHEME}}:";
...
export const listPages = () =>
	[...pages.entries()].map(([name, def]) => ({
		name,
		title: def.title,
		url: `{{INTERNAL_SCHEME}}://${name}`
	}));

//#if popupMenus
export const scheme = "{{INTERNAL_SCHEME}}";
//#endif
export const homeUrl = `{{INTERNAL_SCHEME}}://home`;
...
const errorDocument = (name: string) =>
	wrap(
		...
		`<main class="internal">
       <h1>No such page</h1>
       <p><code>{{INTERNAL_SCHEME}}://${escapeHtml(name)}</code> does not exist.</p>
       <ul>${listPages()
			.map(
				page =>
